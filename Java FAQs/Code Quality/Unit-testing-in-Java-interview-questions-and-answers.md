# Unit testing in Java interview questions and answers
...
coded result. As long as the class being tested behaves as expected, it won’ t notice the dif ference, and the unit test can check that the proper query was emitted.
Here are some reasons why mock objects are handy:
The unit tests as the name implies must test only a unit of the code and not all its collaborating dependencies. Y ou only have to worry about the class
...
Q4 What is the dif ference between a mock object and a stub?
A4 The key dif ference to note is the ability of the mock objects to verify if a particular method was invoked and if yes, how many times was invoked. This is
